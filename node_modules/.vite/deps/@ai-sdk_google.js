import { At as TooManyEmbeddingValuesForCallError, B as postJsonToApi, H as resolve, J as withUserAgentSuffix, M as loadApiKey, Mt as UnsupportedFunctionalityError, Q as _enum, R as parseProviderOptions, S as generateId, X as zodSchema, Y as withoutTrailingSlash, _t as unknown, a as combineHeaders, d as createEventSourceResponseHandler, dt as object, ft as record, g as createProviderToolFactoryWithOutputSchema, gt as union, h as createProviderToolFactory, j as lazySchema, l as convertUint8ArrayToBase64, m as createJsonResponseHandler, mt as string, nt as array, p as createJsonErrorResponseHandler, rt as boolean, s as convertToBase64, ut as number, v as delay, w as getFromApi, yt as AISDKError } from "./dist-COOqKIRy.js";
...
var googleEmbeddingContentPartSchema = union([object({ text: string() }), object({ inlineData: object({
	mimeType: string(),
	data: string()
}) })]);
	...
	for (const { role, content } of prompt) switch (role) {
		...
		case "tool": {
			...
			for (const part of content) {
				...
				if (output.type === "content") if (supportsFunctionResponseParts) appendToolResultParts(parts, part.toolName, output.value);
				else appendLegacyToolResultParts(parts, part.toolName, output.value);
...
var GoogleGenerativeAILanguageModel = class {
	...
	async getArgs({ prompt, maxOutputTokens, temperature, topP, topK, frequencyPenalty, presencePenalty, stopSequences, responseFormat, seed, tools, toolChoice, providerOptions }, { isStreaming = false } = {}) {
		...
		if ((googleOptions == null ? void 0 : googleOptions.serviceTier) && isVertexProvider) sanitizedServiceTier = VertexServiceTierMap[googleOptions.serviceTier];
		const isGemmaModel = this.modelId.toLowerCase().startsWith("gemma-");
		const { contents, systemInstruction } = convertToGoogleGenerativeAIMessages(prompt, {
			isGemmaModel,
			providerOptionsName,
			supportsFunctionResponseParts: this.modelId.startsWith("gemini-3")
		});
	...
	async doGenerate(options) {
		...
		} else if ("functionCall" in part && part.functionCall.name != null && part.functionCall.args != null) content.push({
			type: "tool-call",
		...
		return {
			stream: response.pipeThrough(new TransformStream({
				...
				transform(chunk, controller) {
					...
					if (value.serviceTier != null) serviceTier = value.serviceTier;
...
//#endregion
export { VERSION, createGoogleGenerativeAI, google };
